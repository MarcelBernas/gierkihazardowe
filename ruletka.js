...
function updateBet(chip, betName) {
    if (parseInt(activeElement.id.match(/\d+/g)) > localTokens) {
        ...
        totalbet += chip;
        localTokens -= chip;
        document.getElementById("chips_count").innerText = "Masz " + localTokens + " żetonów. Obecna suma zakładów: " + totalbet + ".";
    }
}
    ...
    if (isRouletteSpinning == false && totalbet > 0) {
        ...
        rouletteStrip.addEventListener('transitionend', function resetPosition() {
            ...
            if (document.cookie.split("; ").find((row) => row.startsWith("chips="))?.split("=")[1] < 1) {
                window.alert("Skończyły się Tobie żetony. Wciśnij 'OK', żeby zresetować swoje postępy.");
